...
# 1. Introduction
This document defines the OpenVPT Protocol, an interoperable mechanism allowing online platforms (Relying Parties) to validate:
...
OpenVPT uses JWT, JOSE, and JWKS for cryptographic security.
...
| Relying Party (RP) | Online service consuming openVPT tokens. |
...
|--------------|---------|-------------|
| **OpenVPT** | Verified Person & Age Token | Main protocol for privacy-preserving proof of real person & age. |
| **IdP** | Identity Provider | Entity that verifies user identity and issues OpenVPT tokens. |
| **RP** | Relying Party | Platform or service that consumes and validates OpenVPT tokens. |
| **JWT** | JSON Web Token | Cryptographically signed token container used by openVPT. |
...
| **API** | Application Programming Interface | Technical interface through which openVPT is implemented. |
| **PII** | Personally Identifiable Information | openVPT avoids sharing these by design. |
...
The OpenVPT specification is currently maintained by its original
...
To enable governance and trust policy enforcement, all OpenVPT proof tokens
...
Issues OpenVPT token.

### POST /verify-token  
Validates OpenVPT token.
...
The Trust Level Framework defines a unified, interoperable, and auditable scale of assurance used across all implementations of the OpenVPT Protocol.  
...
The `trust_level` claim MUST contain an integer **0–4** in openVPT v1.0.  
...
- Reserved for future versions of OpenVPT.
- MUST NOT be issued by any IdP under openVPT v1.0.
...
This mapping ensures openVPT remains interoperable with global digital identity frameworks.
...
- The issuer MAY be suspended from OpenVPT-compatible ecosystems.
...
Introducing TLF 1.0 does **not** change any existing openVPT claim fields.  
...
The openVPT protocol does not restrict which entities may operate as
...
openVPT mitigates this by requiring issuer metadata within each token.
...
openVPT remains a **neutral protocol**:  
...
Examples:
- `openVPT-eu-1.0` — EU-focused profile aligned with eIDAS and regulated KYC providers  
- `openVPT-global-0.1` — minimal global interoperability profile  
- `openVPT-sandbox` — unrestricted profile for development/testing
...
Parties (RPs) implementing the openVPT protocol.
...
- the openVPT token (for short-term replay protection),  
...
- enforce TLS for all openVPT API communication  
...
- isolate openVPT token validation from high-risk backend systems  

---

## 19.5 Data Minimization Principle

openVPT is designed to prevent:
- over-collection,
...
- RPs SHOULD NOT store openVPT tokens beyond what is required for transaction integrity.  
